...
new Component({
  ...
  state: [
    {
      dom: `<input type="text" x-inputmode="numeric" id="js-input-name" class="form__control js-mask-text" placeholder="Ad" maxlength="10">`
    },
    {
      dom: `<input type="text" x-inputmode="numeric" id="js-input-surname" class="form__control js-mask-text" placeholder="Soyad" maxlength="10">`
    },
    {
      dom: `<input type="tel" inputmode="numeric" id="js-input-id" class="form__control js-mask-number" placeholder="TC Kimlik No" maxlength="11">`
    },
    {
      dom: `<div class="price">
                    <input type="tel" inputmode="numeric" id="js-input-home-value" class="form__control js-mask-number js-money-format" placeholder="Ev Değeri" maxlength="12">
                </div>`
    },
    {
      dom: `<div class="price">
                    <input type="tel" inputmode="numeric" id="js-input-loan-amount" class="form__control js-mask-number js-money-format" placeholder="Kredi Tutarı" maxlength="10">
                </div>`
    },
    {
      dom: `<select id="js-input-loan-term" class="form__select">
      <option value="" disabled selected>Vade</option>
    ...
    "#js-button-submit": {
      click: function(e) {
        ...
        if (
          ...
          nameInput.value.trim().length > 10
        ) {
          nameInput.focus();
        } else if (
          surnameInput.value.trim() == "" ||
          surnameInput.value.trim().length < 2 ||
          surnameInput.value.trim().length > 10
        ) {
          surnameInput.focus();
        } else if (
          idInput.value.trim() == "" ||
          idInput.value.trim().length < 11 ||
          idInput.value.trim().length > 11 ||
          idControl(idInput.value.trim()) == false
        ) {
          idInput.focus();
        } else if (
          homeInput.value == "" ||
          parseInt(homeInput.oldValue) < 1000 ||
          parseInt(homeInput.oldValue) > 1000000
        ) {
          homeInput.focus();
        } else if (
          loanInput.value == "" ||
          parseInt(loanInput.oldValue) < 1000 ||
          parseInt(loanInput.oldValue) > 1000000
        ) {
          loanInput.focus();
        } else if (
          parseInt(loanInput.oldValue) >
          (parseInt(homeInput.oldValue) * 75) / 100
        ) {
          loanInput.parents(".form__group")[0].lastChild.innerHTML =
            "Kredi tutarı, ev değerinin %75’inden küçük olmalıdır.(En fazla alabileceğiniz kredi tutarı: " +
            moneyFormat((homeInput.oldValue * 75) / 100 - 1) +
            "TL)";
          loanInput.focus();
        } else {
          let url = `https://www.hesapkurdu.com/konut-kredisi/${
            loanTerm.value
          }-ay-${loanInput.oldValue}-tl?r=${homeInput.oldValue}&n=${
            nameInput.value
          }&s=${surnameInput.value}&t=${idInput.value}`;
          //console.log(url);
...
function idControl(idNumber) {
  ...
  if (
    (singleNumber * 7 - doubleNumber) % 10 !==
    parseInt(idNumber.substr(-2, 1), 10)
  ) {
    return false;
  }
...
function compare(id, errorText) {
  ...
  if (
    document.getElementById(id).nextSibling == null ||
    document.getElementById(id).parentNode.nextSibling == null
  ) {
    errorDom.innerHTML = errorText;
    document
      .getElementById(id)
      .parents(".form__group")[0]
      .appendChild(errorDom);
  } else {
...
//for only text
setInputFilter(document.querySelectorAll(".js-mask-text"), value => {
  return /^[a-zA-Z ]*$/.test(value);
});

//for only number
setInputFilter(document.querySelectorAll(".js-mask-number"), value => {
...
moneyItems(document.querySelectorAll(".js-money-format"), value => {
  return /^[0-9]*$/.test(value);
});
